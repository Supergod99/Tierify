...
public class Tierify implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        ItemGroupEvents.modifyEntriesEvent(RegistryKey.of(RegistryKeys.ITEM_GROUP, new Identifier("ingredients"))).register(content -> {
            ...
            content.addAfter(ItemRegistry.RAW_GALENA, ItemRegistry.CLEANSING_STONE);
        });
        ...
        ModifyItemAttributeModifiersCallback.EVENT.register((itemStack, slot, modifiers) -> {
            ...
            String tierStr = tierTag.getString(Tierify.NBT_SUBTAG_DATA_KEY);
            if (tierStr == null || tierStr.isEmpty()) return;
        
            Identifier tier = new Identifier(tierStr);
            ...
            for (AttributeTemplate template : potentialAttribute.getAttributes()) {
                ...
                // Respect Perfect: skip negative “downsides”
                double amount = template.getEntityAttributeModifier().getValue();
                if (isPerfect && amount < 0.0D) continue;
